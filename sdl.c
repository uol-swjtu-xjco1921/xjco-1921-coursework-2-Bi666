...
void render()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            if (g[i][j] != 0x3f3f3f3f)
            { // 有连接，那么则绘制
                SDL_RenderDrawLine(renderer, graphicPoints[i].x + 5, graphicPoints[i].y + 5, graphicPoints[j].x + 5, graphicPoints[j].y + 5);
            }
        }
    }
    ...
    {
        int p = path[s][pos];
        SDL_RenderDrawLine(renderer, graphicPoints[p].x + 5, graphicPoints[p].y + 5, graphicPoints[pos].x + 5, graphicPoints[pos].y + 5);
        pos = path[s][pos];
    }
    // 绘制基础顶点
    for (int i = 0; i < n; i++)
